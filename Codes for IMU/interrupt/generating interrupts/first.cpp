...
public:
    GPIOInterruptHandler(const std::string& chipName, int lineNum, std::function<void()> callback)
        ...
        interruptLine = chip.get_line(lineNum);  // Get the correct GPIO line
         ...
         if (!interruptLine) {  // Corrected check
...
int main() {
    ...
    auto interruptCallback = [&mpu]() {
        ...
        double accelX = (double)rawAccelX / 16384.0 * 9.81;
        double accelY = (double)rawAccelY / 16384.0 * 9.81;
        double accelZ = (double)rawAccelZ / 16384.0 * 9.81;
        ...
        std::cout << "Acceleration X: " << accelX << " m/s², Y: " << accelY << " m/s², Z: " << accelZ << " m/s²" << std::endl;
        std::cout << "Gyroscope X: " << gyroX << " °/s, Y: " << gyroY << " °/s, Z: " << gyroZ << " °/s" << std::endl;
    };
