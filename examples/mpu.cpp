...
void MPU6050::start() {
    if (running) return;
    running = true;
    dataThread = std::thread(&MPU6050::collectData, this);
    std::cout << "MPU started" << std::endl;
}
...
void MPU6050::stop() {
    ...
    std::cout << "MPU stopped" << std::endl;
}

void MPU6050::collectData() {
    while (running) {
        int16_t accelX = getAccelerationX();
        int16_t accelY = getAccelerationY();
        int16_t accelZ = getAccelerationZ();
